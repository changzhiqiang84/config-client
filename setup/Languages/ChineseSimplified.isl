...
Setting_title=请配置服务器地址
Setting_MasterHost=服务器地址%n格式为 IP址址或域名 + 端口(默认80，可空) + 页面(默认为空)%n如192.168.1.10、192.168.1.10:8080、192.168.1.10:8080/examplepage、www.example.com、www.example.com:8080、www.example.com:8080/examplepage)
Setting_MasterPort=Master端口(0-65535,默认80)
Err_Host=请输入有效的服务器地址
Err_Port=请输入有效的端口(0-65535)
Title_Master_Dir=选择目录路径
Title_Master_CONFIRM_DIR=确定要装数据目录放在这里吗?
Title_Master_CONFIRM_DIR_HINT=请选择数据存储目录，请确保当前磁盘有足够的空间(建议保持默认)
Title_Master_CONFIG_TITLE=自定义配置
Title_Master_CONFIG_PORT=WEB服务端口,请确保据配置的端口未被其它程序占用（0-65535之间）
Title_MASTER_CONFIRM_DELETE_DATA=需要删除此软件系统产生的数据吗?%n包括用户上传文件、数据库、配置数据等
FEATURE_SELECT=功能选择
FEATURE_SELECT_OPTION=请选择您要安装的版本
LED_DROP=LED Drop (支持分辨率自定义及画面位置配置)
LCD_DROP=LCD Drop (本地化，支持超高分辨率)
Additional=附加功能
InstallDSAgent=安装DSAgent(配合PDU中控使用)
FullInstallation=完全安装
CompactInstallation=最小安装
CustomInstallation=自定义安装
Err_NoPSWMIC=当前机器未检测到Powershell,这将影响到程序的正常使用%n继续安装请点击确定，否则将结束安装
